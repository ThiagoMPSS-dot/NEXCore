...
def dump_detailed(stream, end_pos, indent=0):
    while stream.tell() < end_pos:
        ...
        if tid in [0x03, 0x04, 0x05]:
            ...
            print("  " * indent + f"[{hex(tid)}] {name} (Size: {size}, Unk: {unknown})")
            
            if name == "Data" and size > 12000:
                # Dump start of large data to see if there's a header/palette
                blob = stream.read(size)
                print("  " * (indent+1) + f"Dumping Start of 12KB Data: {binascii.hexlify(blob[:64]).decode()}")
            elif tid in [0x03, 0x04]:
                dump_detailed(stream, stream.tell() + size, indent + 1)
            else:
                stream.seek(size, 1)
        elif tid == 0x06:
...
def analyze():
    ...
    dump_detailed(stream, total_size)
